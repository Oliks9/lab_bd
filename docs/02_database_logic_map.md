...
`publish_quiz` требует ровно один у одиночного типа и минимум два варианта всего.
...
Точечное обновление `sql/upgrade_question_options.sql` пересоздаёт тело `pkg_admin`
с проверкой второго правильного варианта и запускает проверку компиляции. Таблицы
и данные не изменяются; требуется существующая актуальная спецификация пакета.
...
- `question_options_smoke_test.sql` проверяет отказ при втором правильном ответе для `SINGLE_CHOICE`/`BOOLEAN`, неизменность числа вариантов и порядка после ошибки, сохранение нескольких правильных для `MULTIPLE_CHOICE`/`ORDERING`, смену типа и защиту публикации для старых некорректных данных. Временные записи откатываются.
