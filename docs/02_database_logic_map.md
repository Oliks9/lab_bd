...
| Процедуры в `pkg_admin` | 27 | `sql/05_packages/pkg_admin.pks/.pkb` |
| Процедуры в `pkg_testing` | 6 | `sql/05_packages/pkg_testing.pks/.pkb` |
| Процедуры в `pkg_reports` | 7 | `sql/05_packages/pkg_reports.pks/.pkb` |
| Триггеры | 5 | `sql/04_triggers/*.sql` |
| Представления | 0 | Не используются; `CREATE VIEW` не требуется |
| Seed-скрипты | 4 | `sql/07_seed/*.sql` |
| Миграции | 5 | `sql/08_migrations/*.sql` |
| Smoke-тесты | 10 | `sql/tests/*smoke*.sql` (отдельно `check_invalid_objects.sql`) |
| Проверка новой установки | 1 | `sql/tests/seed_installation_test.sql` |

## 3. Порядок установки и обновления

Установка также доступна кнопкой первого экрана. Python-модуль `installer.py` выполняет
тот же install.sql через драйвер, без переноса SQL/PLSQL-логики в Python. Порядок команд
не меняется, включая разрушительный `00_uninstall.sql`. Перед выполнением GUI требует
согласие и повтор имени схемы; установщик проверяет права и свободную квоту.
DDL не откатывается целиком. Подробности: `11_installation_from_app.md`.
...
1. `sql/install.sql` удаляет старые объекты (`00_uninstall.sql`), затем создаёт схему в порядке:
   ...
   - триггеры;
   - пакеты;
   - проверка компиляции, включая `pkg_reports`;
   - seed-данные.
2. Для обновления актуальной установленной версии без удаления данных используется `sql/upgrade_no_views.sql`: миграция подбора из `sql/08_migrations` и перекомпиляция пакетов в текущей схеме. Инструкция: `10_deployment_without_views.md`.
...
- `require_question_answers(p_question_id)` проверяет полноту ответа и количество правильных вариантов. Общая проверка для `validate_question` и `publish_quiz`, ошибки `-20109`, `-20143`.
...
- Кто: владелец `DRAFT` теста или `ADMIN`.
- Проверки: у типа вопроса `answer_mode = 'OPTIONS'`; у `SINGLE_CHOICE`/`BOOLEAN` нельзя добавить второй правильный вариант.
- Что делает: добавляет вариант с очередным `seq_no`.
- Ошибки: `-20107`, `-20142`.

Проверка типа и числа правильных ответов выполняется после блокировки строки теста
в `require_quiz_owner`. Два одновременных вызова API не могут сохранить два правильных
ответа одиночного вопроса. `MULTIPLE_CHOICE` и `ORDERING` допускают несколько строк
с `is_correct = 1`. Пакет не делает COMMIT: создание/редактирование вопроса и всех
его вариантов фиксируются клиентом одной транзакцией, при ошибке откатываются целиком.
В процессе составления черновик может временно не иметь правильного ответа;
`publish_quiz` требует ровно один правильный у одиночного типа, минимум два правильных
у множественного типа и минимум два варианта всего для вопросов с вариантами.
Прямой DML владельца схемы не запрещён новой проверкой, но ошибочный вопрос всё равно
отклоняется при публикации через API. Автоматического исправления старых ответов нет.

### `validate_question(p_actor_id, p_question_id)`
- Кто: владелец `DRAFT` теста или `ADMIN`, под блокировкой теста.
- Что делает: вызывает общую `require_question_answers`, ничего не изменяет и не делает COMMIT.
- Правила: `TEXT/NUMBER` требуют непустой эталон; варианты требуют минимум две строки; `SINGLE_CHOICE/BOOLEAN` ровно одну правильную, `MULTIPLE_CHOICE` минимум две, `ORDERING` сохраняет прежнюю проверку наличия правильных строк.
- Ошибки: проверки роли/владельца, `-20123` (вопрос не найден), `-20109` (незаполненный ответ), `-20143` (меньше двух правильных у множественного выбора).

`OracleGateway.create_question` и `update_question` вызывают процедуру после добавления
всех вариантов, но перед COMMIT. Ошибка откатывает вопрос, смену типа и замену вариантов
целиком. При ручной сборке вопроса через SQL следует вызвать её после последнего варианта;
низкоуровневый `add_option` не может требовать два правильных уже на первой вставке.
...
- Проверки:
  ...
  - для `MULTIPLE_CHOICE` минимум 2 правильных варианта.
- Что делает: `quizzes.status = 'PUBLISHED'`.
- Ошибки: `-20108`, `-20109`, `-20143` (а также проверка набора подбора `-20140`).
...
### `set_quiz_access_mode(p_actor_id, p_quiz_id, p_access_mode)`
- Кто: активный автор-владелец черновика или активный `ADMIN` для любого черновика.
- `require_quiz_owner` проверяет роль, владельца и статус `DRAFT`, блокирует строку теста через `FOR UPDATE`.
- Значение нормализуется `UPPER(TRIM(...))`; разрешены только `PUBLIC` и `RESTRICTED`. NULL и неизвестные режимы отклоняются с `-20144`.
- Обновляет только `quizzes.access_mode`; таблицы `quiz_access`, `attempts`, `attempt_questions`, ответы и баллы не затрагивает.
- После публикации доступ проверяет существующая `fn_can_access_quiz`. Ранее выданные приглашения сохраняются и снова действуют после возврата в RESTRICTED.
- Ошибки прав/статуса: `-20100`, `-20101`, `-20102`. Встроенного COMMIT нет.
- Клиентский `OracleGateway.save_quiz_settings` последовательно вызывает `set_quiz_feedback`, `set_quiz_attempt_limit`, `set_quiz_access_mode` и делает один COMMIT. Любая ошибка вызывает общий ROLLBACK. Публикация выполняется следующим отдельным вызовом: если она отклонена, сохранённые настройки остаются у черновика.
- Установка включает процедуру автоматически. Для актуальной существующей схемы `sql/upgrade_quiz_access.sql` пересоздаёт спецификацию/тело пакета и проверяет валидность без изменения данных; CREATE VIEW не нужен.

### `grant_access(p_actor_id, p_quiz_id, p_user_id)`
- Кто: активный автор-владелец теста или `ADMIN`; режим только RESTRICTED, статус может быть DRAFT или PUBLISHED.
- `require_access_manager` проверяет роль, владельца и режим; для изменения блокирует строку теста FOR UPDATE. Для чтения список блокировку не берёт.
- `require_invitation_target` проверяет существование пользователя и запрещает выдачу/отзыв приглашения владельцу или ADMIN: их доступ определяется ролью.
- Добавляет/подтверждает доступ в `quiz_access` через MERGE, повторная выдача не меняет дату и не создаёт дубликат. Явного COMMIT нет.
- Ошибки: `-20100`, `-20110`, `-20145`, `-20146`, `-20147`, `-20148`.

### `list_quiz_access(p_actor_id, p_quiz_id, p_rows OUT SYS_REFCURSOR)`
- Проверяет те же права менеджера закрытого теста, без блокировки/фиксации транзакции.
- Возвращает приглашённых из `quiz_access` и доступ владельца/всех ADMIN по роли без дубликатов.
- Поля курсора: `user_id`, `login`, `full_name`, `role_code`, `is_active`, `access_source` (OWNER/ADMIN/INVITATION), `granted_at`, `can_revoke` (0/1).
- OWNER имеет приоритет над ADMIN. Дата может быть NULL для доступа по роли. Отключённые аккаунты остаются видны с `is_active=0`; это не разрешает им вход.
- Источник списка: таблицы quizzes, app_users, quiz_access. Новые таблицы и представления не создаются.
- Клиент `OracleGateway.quiz_access` преобразует строки курсора в словари и закрывает оба курсора, включая ошибки.

### `revoke_access(p_actor_id, p_quiz_id, p_user_id)`
- После проверок менеджера и целевого пользователя удаляет только соответствующую строку `quiz_access`.
- Повторный отзыв отсутствующего приглашения допустим. Приглашение отключённого участника тоже можно отозвать.
- Не меняет attempts, attempt_questions, ответы, баллы или лимиты. Уже начатая попытка продолжается, новые запрещает существующая `fn_can_access_quiz` в `pkg_testing.start_attempt`.
- Владелец/ADMIN сохраняют доступ по роли; попытка отзыва отклоняется `-20146`.
- Сам пакет не делает COMMIT. `OracleGateway.grant_access/revoke_access` фиксирует успешное изменение либо откатывает ошибку и освобождает блокировки.
- `sql/upgrade_quiz_invitations.sql` подключает актуальный `upgrade_quiz_access.sql`: пересоздаёт спецификацию и тело pkg_admin, проверяет VALID, не изменяет данные. Нужна вся папка sql, CREATE VIEW не требуется.
...
## 11. Пакет отчётности `pkg_reports`

Спецификация: `sql/05_packages/pkg_reports.pks`, тело: `pkg_reports.pkb`.
Все семь процедур открывают выходной `p_rows OUT SYS_REFCURSOR`; данные выбираются
SQL-запросами внутри Oracle. Клиент читает курсор, переводит названия колонок
в нижний регистр и отображает строки. Пакет не изменяет данные и не делает COMMIT.

Для пакета достаточно `CREATE PROCEDURE`. `SYS_REFCURSOR` является встроенным типом.
Типы `catalog_row` (18 полей), `result_row` (21 поле) и `detail_row` (12 полей)
объявлены как PL/SQL RECORD внутри спецификации для FETCH в SQL-проверках;
отдельные схемные типы не создаются, `CREATE TYPE` не требуется.
Нумерация каталогов сохранена: отсутствие `06_views` намеренное.

## 11.1. `pkg_reports.catalog`

Сигнатура: `catalog(p_user_id IN NUMBER, p_topic_id IN NUMBER, p_rows OUT SYS_REFCURSOR)`.
Возвращает только доступные опубликованные тесты по `fn_can_access_quiz`.
`p_topic_id = NULL` означает все тематики; сортировка по тематике и названию.
...
## 11.2. `pkg_reports.history`

Сигнатура: `history(p_user_id IN NUMBER, p_rows OUT SYS_REFCURSOR)`.
Возвращает попытки указанного пользователя во всех статусах, новые сначала.
...
## 11.3. `pkg_reports.attempt_details`

Сигнатура: `attempt_details(p_attempt_id IN NUMBER, p_rows OUT SYS_REFCURSOR)`.
Возвращает только вопросы указанной попытки по `display_order`.
...
Правило действует при прямом вызове процедуры, независимо от виджетов Python.

Колонки выходного курсора:
...
## 11.4. `pkg_reports.question_statistics`

Сигнатура: `question_statistics(p_quiz_id IN NUMBER, p_rows OUT SYS_REFCURSOR)`.
Только вопросы выбранного теста, порядок по ID вопроса. Знаменатель процента:
число сохранённых ответов, а не всех попыток. Без ответов процент равен нулю.
...
## 11.5. `pkg_reports.leaderboard`

Сигнатура: `leaderboard(p_rows OUT SYS_REFCURSOR)`.
Учитывает `FINISHED` и `EXPIRED`; сортировка по среднему, затем лучшему результату
по убыванию, NULL в конце. Пользователи без завершённых/прерванных попыток не входят.
...
## 11.6. `pkg_reports.attempt_result`

Сигнатура: `attempt_result(p_attempt_id IN NUMBER, p_rows OUT SYS_REFCURSOR)`.
Результат указанной попытки и сравнение со средним других участников того же теста.
Возвращает все 16 колонок `history` в том же порядке и пять дополнительных колонок:
...
## 11.7. `pkg_reports.quiz_statistics`

Сигнатура: `quiz_statistics(p_actor_id IN NUMBER, p_rows OUT SYS_REFCURSOR)`.
Активный ADMIN видит все тесты, активный AUTHOR только свои. Для USER, неизвестного
или отключённого аккаунта курсор пуст. Возвращаются в порядке тематик и названий:
`topic_title`, `quiz_title`, `quiz_id`, `attempts_count`, `average_score`.
Учитываются только `FINISHED`/`EXPIRED`; тест без таких попыток имеет количество 0
и среднее NULL. Среднее округляется до двух знаков.

## 11.8. Границы API

Список с нулём строк является нормальным результатом. `attempt_result` для
несуществующего ID возвращает пустой курсор; GUI обращается к существующим попыткам.
Полученные курсоры закрывает вызывающая сторона. `_report_rows` в Python закрывает
оба курсора и при ошибке, не фиксирует и не откатывает чужую транзакцию.
История и детализация принимают ID, а не токен сеанса: как и раньше, подключение
владельцем схемы позволяет прямой SQL. `AUTHID DEFINER` не заменяет аутентификацию
пользователя приложения; эти отчёты не являются отдельной границей защиты схемы.

## 12. Миграции (`sql/08_migrations`)

Точечное обновление `sql/upgrade_question_options.sql` пересоздаёт спецификацию и тело `pkg_admin`
с проверками количества правильных вариантов и запускает проверку компиляции. Таблицы
и данные не изменяются; требуется актуальная установленная структура таблиц.
Полные скрипты установки и обновления также используют новое тело пакета.

Расчёты отчётов и сравнения теперь находятся в `pkg_reports`, без создания представлений.
`sql/upgrade_no_views.sql` вызывает обновление модулей с проверкой компиляции;
таблицы, пароли и попытки сохраняются. Оставшиеся от старой версии представления
не используются новым клиентом; обновление не удаляет их. `00_uninstall.sql`
содержит DROP VIEW только для очистки старых версий при полной переустановке.
...
- `question_options_smoke_test.sql` проверяет отказ при втором правильном ответе для `SINGLE_CHOICE`/`BOOLEAN`, неизменность числа вариантов и порядка после ошибки, минимум два правильных для `MULTIPLE_CHOICE` при итоговой проверке и публикации, сохранение последовательностей, смену типа и старые некорректные данные. Один выбранный участником вариант даёт 0%, полный правильный набор 100%. Временные записи откатываются.

- `reports_smoke_test.sql` проверяет каталог: черновик, закрытый доступ, выданное приглашение, тематику, неизвестного и отключённого пользователя; результат попытки; аналитику ADMIN/AUTHOR/USER, владельца и отключённого автора. Записи откатываются к savepoint.

- `check_invalid_objects.sql` — наличие 32 обязательных объектов (15 таблиц, 4 функции,
  2 процедуры, 3 спецификации и 3 тела пакетов, 5 триггеров) и отсутствие `INVALID`.
  Отсутствующий объект вызывает `ORA-20998`, невалидные объекты — `ORA-20999`.
  В релизном комплекте файл называется `sql/check_installation.sql`.
...
- `-20109` — вопрос не прошёл итоговую проверку или публикацию (ответы/варианты).
...
- `-20142`: у одиночного или логического вопроса уже есть правильный вариант ответа.
- `-20143`: у множественного выбора меньше двух правильных вариантов.
- `-20144`: режим доступа отсутствует или не равен PUBLIC/RESTRICTED.
- `-20145`: приглашения применимы только к закрытому тесту.
- `-20146`: доступ автора/ADMIN определяется ролью, не приглашением.
- `-20147`: целевой пользователь приглашения не найден.
- `-20148`: тест для управления приглашениями не найден.

Проверка `sql/tests/quiz_access_smoke_test.sql` покрывает владельца, администратора,
чужого автора, участника, отключённого автора, запрет правки опубликованного теста,
нормализацию режима, сохранение приглашений и результата 100% при двух переключениях.
Тестовые записи откатываются к SAVEPOINT. Клиентские проверки дополнительно проверяют
общую транзакцию трёх настроек и остановку публикации при ошибке сохранения.
