...
| Seed-скрипты | 3 | `sql/07_seed/*.sql` |
...
18 вопросов, ноль попыток. Скрипт подключён в `sql/install.sql` и
...
не менять пользовательские данные. Все три seed-файла предназначены для новой
установки; выборочное однократное добавление третьего описано в README.

## 14. SQL-проверки и smoke-тесты

- `check_invalid_objects.sql` — контроль, что объекты `VALID`.
- `seed_installation_test.sql` выполняется только сразу после чистой установки: проверяет три начальных аккаунта и их пароли/роли, состав и публикацию тестов, связь категорий с тематикой, пояснения, все шесть типов и прохождение каждого теста на 100%. Проверочные попытки откатываются. Он включён в Docker init и переустановку, но не в обычные проверки/обновление, поскольку пользователь вправе менять демонстрационные данные.
